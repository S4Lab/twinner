...
int main () {
  ...
  const Constraint *con = Constraint::instantiateLessOrEqualConstraint
    (lessOrEqual, exp, new ExpressionTest (new ConcreteValue32Bits (0x1)), 0x1234);
  Logger::debug () << "Constraint: " << con << '\n';
  Logger::info () << "Simplifying the constraint...\n";
  const edu::sharif::twinner::trace::Constraint *simplifiedConstraint =
    edu::sharif::twinner::engine::smt::SmtSolver::getInstance ()->simplifyConstraint (con);
  Logger::debug () << "Simplified constraint: " << simplifiedConstraint << '\n';
